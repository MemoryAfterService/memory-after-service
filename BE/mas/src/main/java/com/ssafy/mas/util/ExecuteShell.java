...
//    private String result_file = "finaldataframe.csv";
    ...
    // 셸 스크립트 실행
    public ArrayList<String> run_shell(String host_dir, String data_name, String result_path, String result_file) {
        String homeDirectory = System.getProperty("user.home");
        Process process;
        ArrayList<String> result = new ArrayList<>();
        try {
            // only for unix
            String command = String.format("bash %s %s %s %s %s %s %s", shell_path, key_path, remote_url, host_dir, data_name, result_path, result_file);
