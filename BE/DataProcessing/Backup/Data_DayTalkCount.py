...
# pyspark 초기 세팅
if len(sys.argv) != 2:
    print("usage : python Data_DayTalkCount.py result/Data_Kakao_line.csv")
else:
    ...
    df.rename(columns = {0:'Username', 1:'Count'},inplace=True)
    df = df.sort_values(by=['Count'], ascending=[False])

    df.to_csv('./result/Data_DayTalkCount_result.csv')
    sc.stop()