...
public class TopKSearch {
	...
	public static class MapClass1 extends Mapper<Object, Text, Text, Text> {
		...
		public void map (Object key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			// TODO
			// ------------------------------------------------------
			emitkey.set(String.format("%s", rid%numOfPartitions));
			emitval.set(value);
	...
	public static class ReduceClass1 extends Reducer<Text, Text, Text, Text> {
			...
			throws IOException, InterruptedException
		{
			...
			double d;
			PriorityQueue<MyType> queue = new PriorityQueue<>();
			// TODO : Find the Top-K closest points from the query point.
			// ------------------------------------------------------
			for (Text t : values) {
				d = dist(query, t.toString());
				MyType e = new MyType(d, t.toString());
			...
			while(!queue.isEmpty()){
				MyType m = queue.poll();
				emitkey.set(m.str);
				emitval.set(Double.toString(m.dist));
				context.write(emitkey, emitval);
			}
			// ------------------------------------------------------
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		// TODO : Fill here for the second phase
		// ------------------------------------------------------
