...
public class CommonItemCount {
	...
	public static class InvertedIndexReducer extends Reducer<Text, Text, Text, IntWritable> {
				...
				throws IOException, InterruptedException
		{
			...
			// TODO : emit < [ridi ridj], 1 > for every pair
			// --------------------------------------------
			for (int i = 0; i < str.size(); i++) {
				for (int j = i+1; j < str.size(); j++) {
					if(str.get(i).compareTo(str.get(j)) < 0){
						ridpair.set(String.format("%s %s", str.get(i), str.get(j)));
					}else {
						ridpair.set(String.format("%s %s", str.get(j), str.get(i)));
					}
	...
	public static class SimMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		public void map( Object key, Text value, Context context ) throws IOException, InterruptedException {
			...
			ridpair.set(String.format("%s %s", st.nextToken(), st.nextToken()));
			// TODO : emit < [ridi ridj], 1 > for every pair
			// --------------------------------------------
			count.set(Integer.parseInt(st.nextToken()));
			// --------------------------------------------
	...
	public static class SimReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
		...
		public void reduce( Text key, Iterable<IntWritable> values, Context context ) throws IOException, InterruptedException {
			// TODO : emit < [ridi ridj], count > for every pair
			// --------------------------------------------
			int sum = 0;
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		// TODO : run phase2 job
		// ------------------------------------------------------
		Job job2 = new Job(conf, "countCommonItems");
