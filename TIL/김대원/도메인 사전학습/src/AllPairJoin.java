...
public class AllPairJoin {
	public static class MapClass1 extends Mapper<Object, Text, Text, Text> {
		...
		public void setup(Context context) throws IOException {
			...
			// TODO : put a data from broadcast inputs
			// ------------------------------------------------------
		...
		public void map(Object key, Text value, Context context)
			...
			// TODO : make a key-value set from data
			// ------------------------------------------------------
			for (int i = 0; i < numberOfPartition; i++) {
				if(tuple[0].equals(Table1name)){
					emitkey.set(String.format("%s %s", partitionId, i));
				}else{
					emitkey.set(String.format("%d %d", i, partitionId));
				}
